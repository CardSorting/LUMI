...
LUMI (`CardSorting.lumi`, `package.json` v2.1.2) is the **agent layer** of the [LUMI monorepo](https://github.com/CardSorting/LUMI): a VS Code extension that plans, proposes, and executes — but never assumes consent. Comfort is UX. **Agency with approval** is architecture.

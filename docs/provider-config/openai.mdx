---
title: "OpenAI"
description: "Learn how to configure and use official OpenAI models with DietCode."
---

DietCode supports accessing models directly through the official OpenAI API.
...
3.  **Create a Key:** Click on "Create new secret key". It's good practice to give your key a descriptive name (e.g., "DietCode API Key").
...
DietCode is compatible with a variety of OpenAI models, including the latest frontier and reasoning models:
...
1.  **Open DietCode Settings:** Click the settings gear icon (⚙️) in the DietCode panel.
...
-   **Azure OpenAI Service:** If you are looking to use the Azure OpenAI service, please note that specific documentation for Azure OpenAI with DietCode may be found separately, or you might need to configure it as an OpenAI-compatible endpoint if such functionality is supported by DietCode for custom configurations.

...
<Steps>
  <Step title="Open DietCode Settings">
    Click the settings gear icon in the top-right corner of the DietCode extension.
  </Step>
  ...
  <Step title="Authorize DietCode">
    In the browser window that opens:
    1. Sign in to your OpenAI account (if not already signed in)
    2. Review the permissions DietCode is requesting
    3. Click **"Authorize"** to grant access
  </Step>
  <Step title="Select Your Model">
    Once authorized, you'll be returned to DietCode. Choose your desired model from the **"Model"** dropdown.
  </Step>
</Steps>
...
To disconnect your OpenAI account from DietCode:
...
OAuth tokens are automatically refreshed by DietCode. If you encounter authentication errors:
...
DietCode only requests the permissions necessary to make API calls on your behalf. Your OpenAI credentials are never stored by DietCode.

When using OpenAI Codex, DietCode accesses models through your OpenAI/ChatGPT subscription. There is no separate per-token API billing or consumption of OpenAI API credits for this provider; usage and limits are governed by your ChatGPT (or OpenAI account) subscription plan, not by DietCode.

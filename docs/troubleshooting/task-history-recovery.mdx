...
description: "How to recover and reconstruct your LUMI task history"
---

Sometimes after an extension update or settings migration you may lose access to previous tasks. This guide covers recovery using the built-in command and storage layout.
...
<Steps>
	...
	<Step title="Run the recovery command">
		Run **LUMI: Reconstruct Task History** (`lumi.reconstructTaskHistory` in `package.json`).
	</Step>
	<Step title="Confirm the action">
		A confirmation prompt will appear. Click **Yes** to proceed.
	</Step>
	<Step title="Wait for reconstruction">
		LUMI will scan your task folders and rebuild the history index.
	</Step>
...
LUMI creates backups automatically. Find them in the `state/` folder:
...
Switching to a new computer? You can bring all your LUMI conversations with you. The process is the same whether you're using VS Code or a JetBrains IDE.

<Steps>
	<Step title="Locate and copy your LUMI data">
	...
	<Step title="Set up your new machine">
		On your **new machine**, install your IDE (VS Code or JetBrains) and the LUMI extension.
	</Step>
	...
	<Step title="Launch and verify">
		Open your IDE. Your task history should now appear in LUMI.
	</Step>
</Steps>
...
Run **"LUMI: Reconstruct Task History"** from the command palette. If that doesn't work, check if there's a backup file to restore.

### History lost after reinstalling VS Code
VS Code usually keeps extension data when uninstalled. Simply reinstall LUMI and run the recovery command.

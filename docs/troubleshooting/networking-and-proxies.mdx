...
description: "Configure DietCode to work behind firewalls and proxies"
---

If you're working behind a corporate proxy or firewall, you'll need to configure
proxy settings for DietCode to connect to AI providers. The configuration varies
depending on which version of DietCode you're using.
...
is needed for DietCode itself.

## CLI

The DietCode CLI uses standard HTTP proxy environment variables. Configure these before running `dietcode` commands.
...
DietCode CLI only supports HTTP proxies. It does not support SOCKS proxies,
...
After configuring the proxy, test that DietCode can connect to your AI provider:

1. Open the DietCode panel
...
DietCode in JetBrains only supports HTTP proxies. It does not support SOCKS
proxies, proxy autoconfiguration (PAC) scripts, or HTTP proxies which require
authentication beyond a basic username and password.

DietCode does not pick up changed proxy settings dynamically. After changing proxy
settings, restart the IDE for DietCode to use the new settings.
...
If you encounter problems with DietCode networking, first verify your proxy

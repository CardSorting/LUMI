...
description: "Authenticate with DietCode and choose your first AI model"
---

DietCode connects to AI models through a **provider**. You have two paths:

<CardGroup cols={2}>
  <Card title="DietCode Provider" icon="bolt">
...
<Steps>
  <Step title="Open Settings">
    Click the settings icon in the top-right of the DietCode panel.
  </Step>

  <Step title="Select a Provider">
    Choose from the **API Provider** dropdown:
    - **DietCode** — simplest setup, no API key needed
  ...
  <Step title="Authenticate">
    **DietCode Provider:** Click **Sign In** and authenticate via Google, GitHub, or email. See [OAuth details](#how-oauth-works) below.
    ...
    **Local:** No key needed — just ensure your local server is running.
    ...
    <Note>
    API keys are stored in your system's credential manager and sent only to your selected provider. They are never logged or transmitted to DietCode's servers.
    </Note>
  </Step>
  ...
  <Step title="Verify">
    Send any message. If DietCode responds, you're ready.
  </Step>
</Steps>

## DietCode Provider

The DietCode Provider gives you one account, one billing relationship, and access to models from Anthropic, OpenAI, Google, and more.
...
Click **Add Credits** in DietCode settings or visit your [account dashboard](https://app.dietcode.bot/dashboard). Credits work across all available models.

### How OAuth Works

<Steps>
  <Step title="Sign In">
    Click **Sign In** in DietCode settings. Your browser opens to `app.dietcode.bot`.
  </Step>
  <Step title="Authenticate">
...
<Note>
Learn more about LLMs and models in [Chapter 2 of AI Coding University](https://dietcode.bot/learn).
</Note>
...
- **Balance & usage:** Open DietCode settings — your credit balance is at the top. Click **View Usage** for transaction history.
- **Switch organization:** Go to DietCode settings → **Switch Organization** to change which billing account is charged.
...
| Browser doesn't open | Check default browser settings. Copy the URL from the DietCode output panel manually. |
...
| Can't access organization | Verify membership at [app.dietcode.bot](https://app.dietcode.bot). Ask your admin about permissions. Sign out and back in. |

## Next Steps

- [Your First Project](/getting-started/your-first-project) — build something with DietCode
- [Core Workflows](/core-workflows/task-management) — patterns you'll use daily
- [Customization](/customization/overview) — tailor DietCode to your workflow

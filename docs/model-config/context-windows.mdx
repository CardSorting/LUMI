...
2. **File contents** - Any files you've shared or DietCode has read
3. **Tool outputs** - Results from executed commands
4. **System prompts** - DietCode's instructions (minimal impact)
...
DietCode can automatically summarize long conversations:
...
### Plan/Act Phase Optimization

Configure separate models for the planning and implementation phases (Settings → "Use different models for Plan and Act modes"). DietCode automatically selects the right model when transitioning between phases:

- **Plan phase**: Use a cost-efficient or reasoning-focused model for exploration
- **Act phase**: Use a coding-optimized model with larger context for implementation
...
**A:** DietCode shows token usage in the interface. Watch for the context meter approaching limits.

### Q: What happens when I exceed the context limit?
**A:** DietCode will either:

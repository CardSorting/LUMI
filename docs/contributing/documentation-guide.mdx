...
description: "How to write and contribute to DietCode documentation"
---

DietCode's documentation lives in the `docs/` directory and uses [Mintlify](https://mintlify.com) for rendering. This guide covers how to write docs that match DietCode's established style.

## Using the Documentation Workflow

The fastest way to create documentation is using the `/write-docs` workflow. Type `/write-docs` in DietCode and describe what you want to document. DietCode guides you through a 4-step process:
...
The workflow file lives at `.dietcoderules/workflows/write-docs.md` and contains templates, style rules, and examples.
...
```markdown
# Good
Switch to bash in DietCode Settings → Terminal → Default Terminal Profile.

# Bad
Users who are experiencing issues may find it helpful to navigate to the 
DietCode settings menu where they can locate the terminal configuration 
...
parts of the codebase. For example, when adding authentication, DietCode 
...
DietCode does things. Files don't get created by DietCode, DietCode creates files.

```markdown
# Good
DietCode reads your project files and builds context automatically.

# Bad
Project files are read and context is built automatically.
```

### Use Neutral Pronouns for DietCode

Refer to DietCode as "it" not "he". DietCode is software, not a person.

```markdown
# Good
When DietCode encounters an error, it suggests fixes.

# Bad
When DietCode encounters an error, he suggests fixes.
```
...
<Frame>
  <img 
    src="https://storage.googleapis.com/dietcode_public_images/docs/assets/filename.png" 
...
<Steps>
  <Step title="Install the Extension">
    Search for "DietCode" in the VS Code marketplace.
  </Step>
...
```jsx
<CardGroup cols={2}>
  <Card title="Getting Started" icon="rocket" href="/getting-started/installing-dietcode">
    Install DietCode and set up your first project.
  </Card>
  <Card title="Features" icon="wand-magic-sparkles" href="/core-workflows/plan-and-act">
    Explore what DietCode can do.
  </Card>
</CardGroup>
```
...
<CardGroup cols={2}>
  ...
  <Card title="Workflows" icon="diagram-project" href="/customization/workflows">
    Learn about DietCode's workflow system.
  </Card>
</CardGroup>

...
description: "Interact with websites through DietCode's controlled browser instance."
---

Browser automation allows DietCode to interact with web content directly through a controlled Chrome instance. It can view websites, test applications, fill forms, and capture screenshots.
...
Ask DietCode to use the browser with simple instructions:
...
DietCode can only run one browser at a time. To visit a different site:
...
The browser has a fixed 900x600 pixel viewport. DietCode shares screenshots after each action so you see exactly what it sees.

### Console Logs

DietCode captures browser console logs, which is helpful for debugging. These are included with each screenshot.
...
2. Search for "DietCode: Chrome Executable Path"

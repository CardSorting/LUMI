...
description: "The Model Context Protocol (MCP) allows DietCode to communicate with external servers that provide additional tools and resources to extend its capabilities. This guide explains how to add and connect to remote MCP servers through the MCP Servers interface."
---
...
To access the MCP Servers interface in DietCode:

1. Click on the DietCode icon in the VSCode sidebar
2. Open the menu (⋮) in the top right corner of the DietCode panel
...
4. DietCode will attempt to connect to the server and display the connection status

> **Note**: When connecting to a remote server, ensure you trust the source, as MCP servers can execute code in your environment.

#### Transport Types

DietCode supports two transport protocols for remote MCP servers:
...
6. You'll be redirected back to DietCode automatically
...
> **Warning**: The following third-party marketplaces are listed for informational purposes only. DietCode does not endorse, verify, or take responsibility for any servers listed on these marketplaces. These servers are cloud-hosted services that process your requests and may have access to data you share with them. Always review privacy policies and terms of use before connecting to third-party services.
...
2. **Request Timeout**:
    - Set how long DietCode should wait for server responses
...
-   **Enabled**: DietCode can use the server's tools and resources
...
-   **Redirect errors**: Verify you're using the latest version of DietCode - older versions may not support OAuth
-   **Reset authentication**: Delete the server and re-add it to start fresh with a new OAuth flow

### Advanced Configuration

For advanced users, DietCode stores MCP server configurations in a JSON file that can be modified:
...
Once connected, DietCode can use the tools and resources provided by the MCP server. When DietCode suggests using an MCP tool:

1. A tool approval prompt will appear (unless auto-approved)
2. Review the tool details and parameters before approving
3. The tool will execute and return results to DietCode
...
5. Sign in to Vercel in your browser and authorize DietCode
6. Return to DietCode - the server will automatically connect
7. Vercel's tools (deploy, logs, projects) are now available to DietCode!

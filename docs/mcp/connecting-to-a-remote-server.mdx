...
description: "The Model Context Protocol (MCP) allows LUMI to communicate with external servers that provide additional tools and resources to extend its capabilities. This guide explains how to add and connect to remote MCP servers through the MCP Servers interface."
---
...
To access the MCP Servers interface in LUMI:

1. Click on the LUMI icon in the VSCode sidebar
2. Open the menu (⋮) in the top right corner of the LUMI panel
...
4. LUMI will attempt to connect to the server and display the connection status

> **Note**: When connecting to a remote server, ensure you trust the source, as MCP servers can execute code in your environment.

#### Transport Types

LUMI supports two transport protocols for remote MCP servers:
...
6. You'll be redirected back to LUMI automatically
7. The server will connect and show a green status dot

Once authenticated, your credentials are securely stored and the server will reconnect automatically when you reload DietCode. You won't need to authenticate again unless you delete the server or your credentials expire.
...
Smithery is a third-party MCP server marketplace that allows users to discover and connect to a variety of Model Context Protocol (MCP) servers. If you're using an MCP-compatible client (such as Cursor, Claude Desktop, or LUMI), you can browse available servers and integrate them directly into your workflow.
...
2. **Request Timeout**:
    - Set how long LUMI should wait for server responses
...
-   **Enabled**: LUMI can use the server's tools and resources
...
-   **Redirect errors**: Verify you're using the latest version of LUMI - older versions may not support OAuth
-   **Reset authentication**: Delete the server and re-add it to start fresh with a new OAuth flow

### Advanced Configuration

For advanced users, LUMI stores MCP server configurations in a JSON file that can be modified:
...
Once connected, LUMI can use the tools and resources provided by the MCP server. When LUMI suggests using an MCP tool:

1. A tool approval prompt will appear (unless auto-approved)
2. Review the tool details and parameters before approving
3. The tool will execute and return results to LUMI
...
5. Sign in to Vercel in your browser and authorize LUMI
6. Return to LUMI - the server will automatically connect
7. Vercel's tools (deploy, logs, projects) are now available to LUMI!

Your Vercel authentication persists across sessions, so you won't need to re-authenticate each time you use DietCode.

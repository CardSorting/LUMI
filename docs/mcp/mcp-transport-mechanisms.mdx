...
description: "Learn about the two primary transport mechanisms for communication between DietCode and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between DietCode and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (DietCode) spawns an MCP server as a child process
...
-   **Locality**: Runs on the same machine as DietCode
...
-   Single-client scenarios (one DietCode instance per server)
...
1. The client (DietCode) connects to the server's SSE endpoint via HTTP GET request
...
-   **Remote Access**: Can be hosted on a different machine from your DietCode instance
...
STDIO servers run locally on the same machine as DietCode, which has several important implications:
...
-   **Execution**: Starts and stops with DietCode (child process lifecycle)
...
-   Start when needed by DietCode
-   Not require network configuration
-   Need to be installed alongside DietCode or via a package manager
...
## Configuring Transports in DietCode

For detailed information on configuring STDIO and SSE transports in DietCode, including examples, see [Configuring MCP Servers](/mcp/configuring-mcp-servers).

...
description: "Learn about the two primary transport mechanisms for communication between LUMI and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between LUMI and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (LUMI) spawns an MCP server as a child process
...
-   **Locality**: Runs on the same machine as LUMI
...
-   Single-client scenarios (one LUMI instance per server)
...
1. The client (LUMI) connects to the server's SSE endpoint via HTTP GET request
...
-   **Remote Access**: Can be hosted on a different machine from your LUMI instance
...
STDIO servers run locally on the same machine as LUMI, which has several important implications:
...
-   **Execution**: Starts and stops with LUMI (child process lifecycle)
...
-   Start when needed by LUMI
-   Not require network configuration
-   Need to be installed alongside LUMI or via a package manager
...
## Configuring Transports in LUMI

For detailed information on configuring STDIO and SSE transports in LUMI, including examples, see [Configuring MCP Servers](/mcp/configuring-mcp-servers).

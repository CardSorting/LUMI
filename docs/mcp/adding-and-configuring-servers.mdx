...
description: "Find MCP servers, add them to DietCode, and configure settings."
---

This guide covers finding MCP servers, adding them to DietCode, and configuring their settings.

## Finding MCP Servers

Several places to find MCP servers:

- **[DietCode MCP Marketplace](/mcp/mcp-marketplace)**: Browse and install directly from DietCode
...
## Adding Servers with DietCode

The easiest way to add an MCP server is to have DietCode build it for you:

1. Provide DietCode with the GitHub repository URL
2. Optionally include the README contents for context
3. DietCode clones the repo, builds it, and adds the configuration

Example conversation:
```text
User: "Add the MCP server from https://github.com/modelcontextprotocol/servers/tree/main/src/brave-search"

DietCode: "Cloning the repository. It needs to be built. Should I run 'npm run build'?"

User: "Yes"

DietCode: "Build complete. This server needs an API key. Where should I find it?"
```

## Managing Servers

Access server settings through the MCP Servers icon in DietCode's top navigation bar.
...
Settings are stored in `dietcode_mcp_settings.json`:
...
4. Find `DietCode>Mcp:Mode` and select your preference

<Frame>
	<img
		src="https://storage.googleapis.com/dietcode_public_images/docs/assets/MCP-settings-edit%20(1).png"
		alt="MCP settings"
	/>
</Frame>
...
After configuring a server, DietCode automatically detects available tools:

1. Type your request in chat
2. DietCode identifies when an MCP tool can help

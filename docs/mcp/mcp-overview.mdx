---
title: "MCP Overview"
description: "Learn about Model Context Protocol (MCP) servers, their capabilities, and how DietCode can help build and use them. MCP standardizes how applications provide context to LLMs, acting like a USB-C port for AI applications."
---
...
DietCode does not come with any pre-installed MCP servers. You'll need to find and install them separately.
...
-   **DietCode Marketplace:** Install one from DietCode's [MCP Marketplace](/mcp/mcp-marketplace)
-   **Ask DietCode:** You can ask DietCode to help you find or create MCP servers
...
## Integration with DietCode

MCP servers work with both the **DietCode VS Code extension** and the **[DietCode CLI](/dietcode-cli/overview)**. If you use the CLI, see [MCP Server Configuration for the CLI](/dietcode-cli/configuration#mcp-server-configuration) to get set up.

DietCode simplifies the building and use of MCP servers through its AI capabilities.

### Building MCP Servers

-   **Natural language understanding:** Instruct DietCode in natural language to build an MCP server by describing its functionalities. DietCode will interpret your instructions and generate the necessary code.
-   **Cloning and building servers:** DietCode can clone existing MCP server repositories from GitHub and build them automatically.
-   **Configuration and dependency management:** DietCode handles configuration files, environment variables, and dependencies.
-   **Troubleshooting and debugging:** DietCode helps identify and resolve errors during development.

### Using MCP Servers

-   **Tool execution:** DietCode seamlessly integrates with MCP servers, allowing you to execute their defined tools.
-   **Context-aware interactions:** DietCode can intelligently suggest using relevant tools based on conversation context.
-   **Dynamic integrations:** Combine multiple MCP server capabilities for complex tasks. For example, DietCode could use a GitHub server to get data and a Notion server to create a formatted report.

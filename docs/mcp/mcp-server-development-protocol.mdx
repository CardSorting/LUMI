...
description: "This protocol is designed to streamline the development process of building MCP servers with DietCode."
---

> **Build and share your MCP servers with the world.** Once you've created a great MCP server, submit it to the [DietCode MCP Marketplace](https://github.com/dietcode/mcp-marketplace) to make it discoverable and one-click installable by thousands of developers.

## What Are MCP Servers?

Model Context Protocol (MCP) servers extend AI assistants like DietCode by giving them the ability to:
...
The heart of effective MCP server development is following a structured protocol. This protocol is implemented through a `.dietcoderules` file that lives at the **root** of your MCP working directory (/Users/your-name/Documents/DietCode/MCP).

### Using `.dietcoderules` Files

A `.dietcoderules` file is a special configuration that DietCode reads automatically when working in the directory where it's placed. These files:

-   Configure DietCode's behavior and enforce best practices
-   Switch DietCode into a specialized MCP development mode
...
Here's the complete MCP Server Development Protocol that should be placed in your `.dietcoderules` file:
...
When this `.dietcoderules` file is present in your working directory, DietCode will:
...
### 1. Create a `.dietcoderules` file (IMPORTANT)

First, add a `.dietcoderules` file to the root of your MCP working directory using the protocol above. This file configures DietCode to use the MCP development protocol when working in this folder.

### 2. Start a Chat with a Clear Description

Begin your DietCode chat by clearly describing what you want to build. Be specific about:
...
DietCode will automatically start in PLAN MODE, guiding you through the planning process:
...
When planning is complete, DietCode presents the plan in chat and **automatically** transitions to implementation — no manual mode switch required.

### 4. Provide API Documentation Early

One of the most effective ways to help DietCode build your MCP server is to share official API documentation right at the start:
...
Providing comprehensive API details (endpoints, authentication, data structures) significantly improves DietCode's ability to implement an effective MCP server.

## Understanding the Two Modes

### PLAN MODE

In this collaborative phase, you work with DietCode to design your MCP server:
...
Once planning is complete, DietCode helps implement the server:
...
<Frame>
	<img
		src="https://storage.googleapis.com/dietcode_public_images/docs/assets/planning-phase.gif"
...
<Frame>
	<img
		src="https://storage.googleapis.com/dietcode_public_images/docs/assets/building-mcp-plugin.gif"
...
Resources let your MCP servers expose data to DietCode without executing code. They're perfect for providing context like files, API responses, or database records that DietCode can reference during conversations.
...
Resources make your MCP servers more context-aware, allowing DietCode to access specific information without requiring you to copy/paste. For more information, refer to the [official documentation](https://modelcontextprotocol.io/docs/concepts/resources).

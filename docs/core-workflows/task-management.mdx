...
Every interaction with LUMI happens within a task. Tasks are self-contained work sessions that capture your entire conversation, code changes, command executions, and decisions.

## What are Tasks?

A task begins when you submit a prompt to LUMI. Your prompt defines the goal, and LUMI works toward it through conversation, code changes, and tool use. The quality of your initial prompt directly affects how well LUMI performs - clear, specific prompts lead to better results.
...
<Tip>
Want to get better results from LUMI? Learn how to write effective prompts in our [Prompt Module](https://dietcode.bot/learn).
</Tip>

## Scoping Your Tasks

Each task carries its own context: the conversation history, decisions made, and understanding built up over the session. How you scope your tasks directly affects how well LUMI can help you.
...
Every AI model has a context window - a limit on how much information it can process at once. Think of it as LUMI's working memory for the current task.

As you work, the context window fills up with:
- Your prompts and LUMI's responses
- File contents LUMI reads or edits
- Command outputs and tool results
- System instructions that guide LUMI's behavior (including [DietCode Rules](/customization/dietcode-rules))

When the context window approaches its limit, LUMI automatically compresses older parts of the conversation to make room. This means very long tasks may lose some earlier details, though LUMI preserves the most important context.
...
Knowing when to start fresh versus continue can feel unclear at first. As you work with LUMI more, you'll develop an intuition for it. Use this table as a starting point:

| Scenario | Action | Why |
|----------|--------|-----|
| Switching to a different feature | **New task** | Clean context, focused responses |
| Building on work LUMI just completed | **Continue** | Shared understanding preserved |
| LUMI keeps going off-track | **New task** | Fighting context wastes time |
...
| Refining LUMI's last output | **Continue** | Momentum and decisions preserved |

To start a new task, click the **+** button in the LUMI sidebar or use the `/newtask` command. Your file changes are preserved through [checkpoints](/core-workflows/checkpoints), and you can reference previous tasks from history anytime.

## Understanding Task Costs

Every cloud-based AI model charges for usage based on tokens, the units of text the model processes. LUMI tracks these costs automatically and displays them in the task header so you can monitor spending as you work.

### How Costs Are Calculated

When you interact with LUMI, the model processes:
...
|--------------|---------------|
| **LUMI Provider** | Pay-per-use with credits you purchase |
...
Not ready to pay? LUMI offers several free paths:

- **Free models**: Search "free" in the model selector when using the LUMI provider. These models display a **FREE** tag and work well for learning and experimentation.
...
Click the **History** button in the LUMI sidebar (clock icon at the top-right) to open the history view. You'll see all your past tasks with their initial prompt, timestamp, and token usage. Each task card expands to show a preview of the conversation.

### Searching Tasks

Use the search bar at the top of the history view to find specific tasks. The fuzzy search looks across everything: your prompts, LUMI's responses, code snippets, and file names. 
...
LUMI can resume interrupted tasks with full context:

1. Open the task from history
2. LUMI loads the complete conversation
...
This works across sessions. Even if you close the editor and return days later, LUMI can pick up where you left off.

...
New models drop constantly, so this guide focuses on what's working well with DietCode right now. We'll keep it updated as the landscape shifts.

<Callout type="tip">
**New to model selection?** Start with [Module 2 of DietCode's Learning Path](https://dietcode.bot/learn) for a comprehensive guide to choosing and configuring models.
</Callout>

## What is an AI Model?

Think of an AI model as the "brain" that powers DietCode. When you ask DietCode to write code, fix bugs, or refactor your project, it's the model that actually understands your request and generates the response.
...
- **You choose which model DietCode uses** like picking between different experts for different tasks
- **Models are accessed via API providers** - companies like Anthropic, OpenAI, and OpenRouter host these models

**Why it matters:** The model you choose directly impacts DietCode's capabilities, response quality, speed, and cost. A premium model might handle complex refactoring beautifully but cost more, while a budget model works great for routine tasks at a fraction of the price.

## How to Select a Model in DietCode

Follow these 5 simple steps to get DietCode up and running with your preferred AI model:

### Step 1: Open DietCode Settings

First, you need to access DietCode's configuration panel.

**Two ways to open settings:**
- **Quick method**: Click the **gear icon (⚙️)** in the top-right corner of DietCode's chat interface
- **Command palette**: Press **Cmd/Ctrl + Shift + P** → type "DietCode: Open Settings"

<Frame>
  <img src="https://storage.googleapis.com/dietcode_public_images/step1-config.png" alt="DietCode Settings Panel" />
</Frame>
...
<Frame>
  <img src="https://storage.googleapis.com/dietcode_public_images/step2-provider.png" alt="DietCode Settings Panel" />
</Frame>
...
|----------|----------|-------|
| **DietCode** | Easiest setup | No API keys needed, access to multiple models including stealth models |
...
See the [full provider list](/getting-started/authorizing-with-dietcode) for all 30+ supported providers including xAI Grok, Mistral, Groq, Fireworks, Together, Baseten, SambaNova, Nebius, Hugging Face, and more.

<Info>
**Recommended for beginners:** Start with **DietCode** as your provider - no API key management needed, instant access to multiple models, and occasional free inferencing through partner providers.
</Info>
...
#### If you selected **DietCode** as your provider:

- **No API key needed!** Simply sign in with your DietCode account
- Click the **Sign In** button when prompted
- You'll be redirected to [app.dietcode.bot](https://app.dietcode.bot) to authenticate
- After signing in, return to your IDE

<Note>
  For detailed information about the DietCode authentication flow, OAuth tokens, and troubleshooting, see [Authorizing with DietCode](/getting-started/authorizing-with-dietcode).
</Note>

#### If you selected **OpenAI Codex** as your provider:

- **No API key needed!** If you have a ChatGPT subscription (Plus, Pro, or Team), you can use it directly in DietCode
...
1. **Visit your provider's website to get an API key:**
   ...
   - **Others**: See [Provider Setup Guide](/getting-started/authorizing-with-dietcode)

2. **Generate a new API key** on the provider's website

3. **Copy the API key** to your clipboard

4. **Paste your key** in the **"API Key"** field in DietCode settings

5. **Save automatically** - Your key is stored securely in your editor's secrets storage

<Frame>
  <img src="https://storage.googleapis.com/dietcode_public_images/step3-API.png" alt="DietCode API Selection" />
</Frame>
...
<Frame>
  <img src="https://storage.googleapis.com/dietcode_public_images/step4-model.png" alt="DietCode Model Selection" />
</Frame>
...
### Step 5: Start Using DietCode

**Congratulations! You're all set up.** Here's how to start coding with DietCode:

1. **Type your request** in the DietCode chat box

...
title: "Hooks"
sidebarTitle: "Hooks"
description: "Inject custom logic into DietCode's workflow to validate operations and shape DietCode's decisions."
---

Hooks are scripts that run at key moments in DietCode's workflow. Because they execute at known points with consistent inputs and outputs, hooks bring determinism to the non-deterministic nature of AI models by enforcing guardrails, validations, and context injection. You can validate operations before they execute, monitor tool usage, and shape how DietCode makes decisions.
...
- Add context to the conversation based on what DietCode is doing

## Hook Types

DietCode supports 8 hook types that run at different points in the task lifecycle:
...
| PreToolUse | Before DietCode executes a tool (read_file, write_to_file, etc.) |
| PostToolUse | After a tool execution completes |
| UserPromptSubmit | When you submit a message to DietCode |
| PreCompact | Before DietCode truncates conversation history to free up context |
...
flowchart TD
    ...
    subgraph Conversation Cycle
        ...
        H_Submit --> Thinking[DietCode Processes Context]:::state
...
2. **Conversation Cycle**: Each time you send a message, **UserPromptSubmit** runs, then DietCode processes your request
3. **Tool Execution**: When DietCode decides to use a tool, **PreToolUse** runs first-if allowed, the tool executes, then **PostToolUse** runs
...
- **Global hooks**: `~/Documents/DietCode/Hooks/`
- **Project hooks**: `.dietcoderules/hooks/` in your repo (can be committed to version control)
...
<Steps>
  <Step title="Open the Hooks tab">
    Click the scale icon at the bottom of the DietCode panel, to the left of the model selector. Switch to the Hooks tab.
  </Step>
  ...
  <Step title="Review the hook's code">
    Click the pencil icon to open and edit the hook script. DietCode generates a template with examples.
  </Step>
...
Let's create a simple hook that logs every file DietCode reads or writes. You'll see results in seconds.
...
```bash
#!/bin/bash
# Logs all file operations to ~/dietcode-activity.log
...
# Log to file
echo "$(date '+%H:%M:%S') - $TOOL: $FILE_PATH" >> ~/dietcode-activity.log
...
<Steps>
  <Step title="Create the hook file">
    Save the script above as `~/Documents/DietCode/Hooks/file-logger` or create it through the Hooks UI.
  </Step>
  <Step title="Make it executable">
    On macOS/Linux, run `chmod +x ~/Documents/DietCode/Hooks/file-logger`.
  </Step>
  <Step title="Enable it (macOS/Linux only)">
    In DietCode's Hooks tab, find "file-logger" under PreToolUse hooks and toggle it on.
  </Step>
</Steps>
...
Ask DietCode to read any file in your project: "What's in package.json?"

Then check the log:

```bash
cat ~/dietcode-activity.log
```
...
| `contextModification` | string | Optional text that gets injected into the conversation as context for DietCode |
...
- Providing validation results that DietCode should consider
...
Runs when you send a message to DietCode. Use it to:
...
Runs before DietCode truncates conversation history to stay within context limits. Use it to:
...
echo "$(date -Iseconds) | $TOOL | success=$SUCCESS | ${DURATION}ms" >> ~/.dietcode-tool-log.txt
...
Hooks are available in the [DietCode CLI](/dietcode-cli/getting-started):

```bash
# Enable hooks for a task
dietcode "What does this repo do?" -s hooks_enabled=true

# Configure hooks globally
dietcode config set hooks-enabled=true
```
...
- [Rules](/customization/dietcode-rules) define high-level guidance that hooks can enforce programmatically

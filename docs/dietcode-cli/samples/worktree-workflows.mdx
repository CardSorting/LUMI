...
description: "Use Git worktrees with DietCode CLI to run parallel tasks, test different approaches, and pipe context between isolated environments"
---

Git worktrees let you have multiple branches checked out simultaneously in different folders. Combined with DietCode CLI's `--cwd` flag, this enables powerful parallel development workflows and isolated experimentation.
...
The `-c, --cwd <path>` flag tells DietCode to run in a specific directory without changing your current location:

```bash
# Run DietCode in a different directory
dietcode --cwd ~/worktree-a -y "refactor the authentication code"

# Short form
dietcode -c ~/worktree-b -y "add unit tests"
```

This is the key to worktree workflows—you can run multiple DietCode instances in different worktrees simultaneously from a single terminal.
...
dietcode --cwd ~/worktree-a -y "read the last 10 changes using git show and update our README with them" &

# Terminal 2: TypeScript migration in worktree-b
dietcode --cwd ~/worktree-b -y "update the index.js to use typescript" &

# Terminal 3: Refactoring in worktree-c
dietcode --cwd ~/worktree-c -y "refactor the cli/ folder to be more modular" &
...
dietcode --cwd ~/worktree-a -y \
  ...
  | dietcode --cwd ~/worktree-b -y \
  "i've tried to edit the index.ts in a different worktree but it failed. use a different approach for this work tree"
```

**How it works:**
1. First DietCode instance runs in `worktree-a`, attempts a change, tests it
2. If it fails, outputs just the failure summary
3. That summary is piped to a second DietCode instance in `worktree-b`
...
dietcode --cwd ~/worktree-docs --config ~/.dietcode-haiku -y \
  "update README with latest changes"

# Expensive model for complex refactoring
dietcode --cwd ~/worktree-refactor --config ~/.dietcode-opus --thinking -y \
...
dietcode --cwd ~/worktree-a -y "long-running task"
dietcode --cwd ~/worktree-b -y "another task"
```
...
dietcode --cwd ~/worktree-a "refactor code"

# With -y: Runs autonomously (doesn't block)
dietcode --cwd ~/worktree-a -y "refactor code" &
```
...
git worktree add ~/dietcode-worktrees/feature-auth -b feature/authentication
git worktree add ~/dietcode-worktrees/feature-api -b feature/api-endpoints
git worktree add ~/dietcode-worktrees/fix-tests -b fix/failing-tests

# Pattern 1: Run parallel independent tasks
dietcode -c ~/dietcode-worktrees/feature-auth -y --config ~/.dietcode-sonnet \
  "implement JWT authentication" &

dietcode -c ~/dietcode-worktrees/feature-api -y --config ~/.dietcode-sonnet \
  "create REST API endpoints for user management" &

dietcode -c ~/dietcode-worktrees/fix-tests -y --config ~/.dietcode-haiku \
...
dietcode -c ~/dietcode-worktrees/feature-auth -y \
...
cd ~/dietcode-worktrees/feature-auth
git checkout main
git merge feature/authentication

# Cleanup
git worktree remove ~/dietcode-worktrees/feature-auth
```
...
<AccordionGroup>
  <Accordion title="Worktree Organization">
    - **Use a dedicated folder**: Create `~/dietcode-worktrees/` for all worktrees
...
<AccordionGroup>
  ...
  <Accordion title="Tasks not running in parallel">
    ...
    - `-y` flag so DietCode doesn't wait for approval
...
<Columns cols={2}>
  ...
  <Card title="Model Orchestration" icon="layer-group" href="/dietcode-cli/samples/model-orchestration">

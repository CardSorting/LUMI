...
description: "Automated GitHub issue analysis using DietCode CLI to identify root causes."
---

Automated GitHub issue analysis using DietCode CLI. This script uses DietCode's autonomous AI capabilities to fetch, analyze, and identify root causes of GitHub issues, outputting clean, parseable results that can be easily integrated into your development workflows.

<Note>
**New to DietCode CLI?** This sample assumes you have already completed the [Installation Guide](https://docs.dietcode.bot/dietcode-cli/installation) and authenticated with `dietcode auth`. If you haven't set up DietCode CLI yet, please start there first.
</Note>

<Frame>
  <img src="https://storage.googleapis.com/dietcode_public_images/cli-rca.gif" alt="CLI Root Cause Analysis Demo" width="600" />
</Frame>
...
- **DietCode CLI** installed and authenticated ([Installation Guide](https://docs.dietcode.bot/dietcode-cli/installation))
- **At least one AI model provider** configured (e.g., OpenRouter, Anthropic, OpenAI)
- **Basic familiarity** with DietCode CLI commands
...
```bash
curl -O https://raw.githubusercontent.com/dietcode/dietcode/main/src/samples/cli/github-issue-rca/analyze-issue.sh
```
...
### Using Specific DietCode Instance

Target a particular DietCode instance by address:
...
<Warning>
This is useful when:
- Running multiple DietCode instances
- Using a remote DietCode server
- Testing with specific configurations
</Warning>

<Note>
The script will automatically handle everything: fetching the issue, analyzing it with DietCode, and displaying the results. The analysis typically takes 30-60 seconds depending on the issue complexity.
</Note>
...
- Supports optional DietCode instance address
...
This sample demonstrates how to build an autonomous GitHub issue analysis tool using DietCode CLI:

1. **Building autonomous CLI tools** using DietCode's capabilities
2. **Parsing structured JSON output** from DietCode CLI
...
- [CLI Installation Guide](https://docs.dietcode.bot/dietcode-cli/installation)
- [CLI Reference Documentation](https://docs.dietcode.bot/dietcode-cli/cli-reference)
- [Headless Mode](https://docs.dietcode.bot/dietcode-cli/three-core-flows)

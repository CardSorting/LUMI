---
title: "Configuration"
description: "Manage DietCode CLI settings with dietcode config, environment variables, and configuration files"
---

DietCode CLI provides multiple ways to configure settings, from the interactive `dietcode config` command to environment variables for automation.
...
Manage DietCode rules that guide AI behavior:

- **`.dietcoderules` files**: Project-specific rules in your workspace
- **Cursor rules**: Import rules from Cursor editor format
- **Windsurf rules**: Import rules from Windsurf editor format

Rules help DietCode understand your project's conventions, coding standards, and preferences.
...
- Hooks run at key points in DietCode's workflow

<Note>
Hooks must be enabled via settings. Use `dietcode config` to toggle `hooks-enabled`.
</Note>

### Skills Tab

Manage [skills](/customization/skills) that extend DietCode's capabilities:
...
DietCode stores configuration in `~/.dietcode/data/`:
...
│   │   └── dietcode_mcp_settings.json  # MCP server configuration
...
```bash
export CLINE_DIR=/custom/path/to/dietcode
dietcode "your task"
```

When set, all DietCode data is stored in this directory instead of `~/.dietcode/data/`.

**Use cases:**
- Running multiple isolated DietCode configurations
...
Run DietCode with a custom configuration directory:

```bash
dietcode --config /path/to/custom/config "your task"
```

This is useful for:
- Running isolated DietCode instances
...
```bash
# Work configuration
dietcode --config ~/.dietcode-work "review this PR"

# Personal projects
dietcode --config ~/.dietcode-personal "help me with this side project"
```

## MCP Server Configuration

DietCode CLI supports [MCP (Model Context Protocol)](/mcp/mcp-overview) servers, giving you access to external tools and data sources directly from the terminal. The CLI uses the same MCP configuration format as the VS Code extension.
...
```
~/.dietcode/data/settings/dietcode_mcp_settings.json
```
...
<Note>
The CLI does not yet have a `/mcp` slash command for managing MCP servers interactively. For now, you'll need to edit the `dietcode_mcp_settings.json` file directly.
</Note>

### Custom Config Directory

If you use the `CLINE_DIR` environment variable or `--config` flag, the MCP settings file will be located at `<your-config-dir>/data/settings/dietcode_mcp_settings.json` instead.
...
dietcode -m ollama/llama3 "your task"
```
...
If you use the DietCode VS Code extension, the CLI automatically detects and can share some settings. However, the CLI maintains its own configuration for terminal-specific features.

### From Other CLI Tools

See [Installation & Setup](/dietcode-cli/installation#option-3-import-from-existing-tools) for importing configurations from:
- Codex CLI
- OpenCode
...
echo "Use TypeScript strict mode" > .dietcoderules/typescript.md
```
...
```bash
# Commit .dietcoderules/ to your repo
git add .dietcoderules/
git commit -m "Add DietCode rules for team"
```
...
1. Check write permissions on `~/.dietcode/data/`

...
DietCode works with VSCode's multi-root workspaces, letting you manage multiple project folders or repositories in a single window. Whether you're working with a monorepo or separate Git repositories, DietCode can read files, write code, and run commands across all of them.

<Frame>
  <video
    src="https://storage.googleapis.com/dietcode_public_images/multiworkspace.mp4"
    autoPlay
    muted
    loop
    playsInline
...
- **DietCode rules** only work in the primary workspace folder
...
DietCode can complete tasks that span multiple projects or repositories:
...
DietCode supports both patterns, as well as hybrid setups where some folders are Git repositories and others are not. The key difference: with multiple repositories, each folder has its own `.git` directory and DietCode tracks them independently.
...
When you open separate Git repositories in one workspace, DietCode treats each as an independent project with its own version control.

### What DietCode Tracks Per Repository

For each workspace folder, DietCode detects:
...
This means DietCode understands that your frontend and backend might be at different commits, on different branches, or even use different version control systems.

<Note>
While DietCode detects VCS information for all workspace folders, certain features only use the **primary workspace** (the first folder): [DietCode rules](/customization/dietcode-rules), [workflows](/customization/workflows), and [Git-related features](/core-workflows/working-with-files) like `@git` mentions.
</Note>
...
DietCode understands natural references to your workspaces:
...
- DietCode needs to resolve ambiguity
...
Each service has its own repository. DietCode can update the proto definitions and regenerate clients across all services.
...
The types repository defines interfaces used by both client and server. When you update a type, DietCode can fix both consumers.
...
Mix of repositories and plain folders. DietCode adapts to each folder's configuration.
...
[DietCode rules](/customization/dietcode-rules) (`.dietcoderules/` directory) only work in the **primary workspace** (the first folder in your workspace). Rules in other workspace folders are ignored.

**Workaround:** Place shared rules in the primary workspace, or use global rules (`~/Documents/DietCode/Rules/`) which apply everywhere.

### Checkpoints

[Checkpoints](/core-workflows/checkpoints) are disabled in multi-root workspace mode. DietCode displays a warning when this happens.
...
4. **Consider the primary workspace** for DietCode rules placement
...
- Use [Plan mode](/core-workflows/plan-and-act) to let DietCode understand structure first
- Add a `.dietcodeignore` file to reduce noise, speed up scanning, and keep DietCode focused on source code:
...
For more patterns and gotchas, see the [.dietcodeignore File Guide](/customization/dietcodeignore).

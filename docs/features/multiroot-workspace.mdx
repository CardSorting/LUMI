...
LUMI works with VSCode's multi-root workspaces, letting you manage multiple project folders or repositories in a single window. Whether you're working with a monorepo or separate Git repositories, LUMI can read files, write code, and run commands across all of them.
...
- **LUMI rules** only work in the primary workspace folder
...
LUMI can complete tasks that span multiple projects or repositories:
...
LUMI supports both patterns, as well as hybrid setups where some folders are Git repositories and others are not. The key difference: with multiple repositories, each folder has its own `.git` directory and LUMI tracks them independently.
...
When you open separate Git repositories in one workspace, LUMI treats each as an independent project with its own version control.

### What LUMI Tracks Per Repository

For each workspace folder, LUMI detects:
...
This means LUMI understands that your frontend and backend might be at different commits, on different branches, or even use different version control systems.
...
LUMI understands natural references to your workspaces:
...
- LUMI needs to resolve ambiguity
...
Each service has its own repository. LUMI can update the proto definitions and regenerate clients across all services.
...
The types repository defines interfaces used by both client and server. When you update a type, LUMI can fix both consumers.
...
Mix of repositories and plain folders. LUMI adapts to each folder's configuration.
...
**Workaround:** Place shared rules in the primary workspace, or use global rules (`~/Documents/LUMI/Rules/`) which apply everywhere.

### Checkpoints

[Checkpoints](/core-workflows/checkpoints) are disabled in multi-root workspace mode. LUMI displays a warning when this happens.
...
4. **Consider the primary workspace** for LUMI rules placement
...
- Use [Plan mode](/core-workflows/plan-and-act) to let LUMI understand structure first

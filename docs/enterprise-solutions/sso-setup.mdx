...
description: "Configure Single Sign-On (SSO) for DietCode Enterprise via WorkOS AuthKit."
---

## Overview
DietCode Enterprise integrates with your identity provider (IdP) via **WorkOS AuthKit** for SSO.

This page describes, at a high level, how SSO is set up for DietCode Enterprise using WorkOS AuthKit.
...
1) **DietCode Dashboard (app.dietcode.bot)**
...
## Using the DietCode Dashboard
Use the DietCode Dashboard at https://app.dietcode.bot to:
...
To connect your IdP to DietCode Enterprise, configure your identity provider in **WorkOS AuthKit**:
...
DietCode Enterprise’s default SSO integration is **via WorkOS**.
...
1) Attempt an SSO sign-in from https://app.dietcode.bot.
2) Confirm the sign-in completes (you are redirected back successfully).

## Troubleshooting
- **Redirect URI mismatch**: confirm the redirect/callback URL configured in WorkOS matches what was provided during your DietCode Enterprise onboarding.

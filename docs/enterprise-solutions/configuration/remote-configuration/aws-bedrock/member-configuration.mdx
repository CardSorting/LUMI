...
**DietCode extension installed and configured**  
The DietCode extension must be installed in VS Code and you need to be signed into your organization account. If you haven't installed DietCode yet, follow our [installation guide](/getting-started/installing-dietcode).

<Info>
**Quick Check**: Open the DietCode panel in VS Code. If you see your organization name in the bottom left, you're signed in correctly.
</Info>
...
<Frame>
	<img
		src="https://assets.int.dietcode.bot/assets/VS%20Code%20Bedrock%20API%20Key.gif"
	/>
</Frame>
...
<Steps>
<Step title="Open DietCode Settings">
Open VS Code and access the DietCode settings panel using either of these methods:

- Click the settings icon (⚙️) in the DietCode panel
...
3. DietCode will use the credentials associated with that profile
</Accordion>
...
2. DietCode will automatically detect credentials from your environment using the standard AWS credential provider chain
</Accordion>
</AccordionGroup>
...
Send a test message in DietCode to verify your credentials work correctly with the configured Bedrock region.
...
Confirm you're signed into the correct DietCode organization. Verify your administrator has saved the Bedrock configuration. Try signing out and back into the extension.

...
description: "Optional telemetry and observability for your DietCode deployment"
---

DietCode includes optional monitoring capabilities for organizations that want to track usage and integrate with their observability infrastructure.
...
<CardGroup cols={1}>
  <Card title="DietCode Telemetry" icon="chart-simple" href="/enterprise-solutions/monitoring/telemetry">
    Built-in anonymous usage tracking that helps improve DietCode (opt-in)
  </Card>  
</CardGroup>

## DietCode Telemetry

DietCode includes opt-in telemetry for anonymous usage tracking:
...
Users can enable or disable telemetry in DietCode settings. All data is anonymous and does not include code content, file paths, or sensitive information.

See [DietCode Telemetry](/enterprise-solutions/monitoring/telemetry) for configuration details.

## OpenTelemetry Integration

For advanced monitoring needs, DietCode supports OpenTelemetry's OTLP (OpenTelemetry Protocol) for exporting metrics and logs to your own infrastructure.
...
### When to Use DietCode Telemetry
- You want to help improve DietCode through anonymous usage data
...
For basic telemetry, enable it in DietCode settings. For OpenTelemetry, see the configuration guide.
</Step>
...
All DietCode monitoring features are designed with privacy in mind:

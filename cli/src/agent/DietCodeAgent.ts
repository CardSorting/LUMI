/**
 * DietCodeAgent - Decoupled ACP Agent implementation for DietCode CLI.
 *
 * This class implements the ACP (Agent Client Protocol) Agent interface,
 * allowing DietCode to be used programmatically without stdio dependency.
...
import type { DietCodeMessageChange } from "@core/task/message-state"
...
import type { DietCodeAsk, DietCodeMessage as DietCodeMessageType } from "@shared/ExtensionMessage"
import { CLI_ONLY_COMMANDS, VSCODE_ONLY_COMMANDS } from "@shared/slashCommands"
import { getProviderModelIdKey } from "@shared/storage/provider-keys"
import { DietCodeEndpoint } from "@/config.js"
...
import { DietCodeSessionEmitter } from "./DietCodeSessionEmitter.js"
import { translateMessage } from "./messageTranslator.js"
import { handlePermissionResponse } from "./permissionHandler.js"
import type { DietCodeAcpSession, DietCodeAgentOptions, PermissionHandler } from "./public-types.js"
...
/**
 * DietCode's implementation of the ACP Agent interface.
 *
 * This agent bridges the ACP protocol with DietCode's core Controller,
 ...
 * For stdio-based ACP communication, use the AcpAgent wrapper class.
 */
export class DietCodeAgent implements acp.Agent {
	private readonly options: DietCodeAgentOptions
	private readonly ctx: CliContextResult

	/** Map of active sessions by session ID */
	public readonly sessions: Map<string, DietCodeAcpSession> = new Map()

	/** WeakMap to associate DietCodeAcpSession with its Controller without exposing it to consumers */
	readonly #sessionControllers = new WeakMap<DietCodeAcpSession, Controller>()
	...
	private readonly sessionEmitters: Map<string, DietCodeSessionEmitter> = new Map()
	...
	constructor(options: DietCodeAgentOptions) {
		this.options = options
		this.ctx = initializeCliContext({ dietcodeDir: options.dietcodeDir })
	}
	 ...
	 */
	emitterForSession(sessionId: string): DietCodeSessionEmitter {
		let emitter = this.sessionEmitters.get(sessionId)
		if (!emitter) {
			emitter = new DietCodeSessionEmitter()
	...
	async initialize(params: acp.InitializeRequest, connection?: acp.AgentSideConnection): Promise<acp.InitializeResponse> {
		...
		await DietCodeEndpoint.initialize(this.ctx.EXTENSION_DIR)
		...
		return {
			...
			authMethods: [
				{
					id: "dietcode-oauth",
					name: "Sign in with DietCode",
					description: "Authenticate with your DietCode account via browser OAuth",
				},
				{
	...
	async newSession(params: acp.NewSessionRequest): Promise<acp.NewSessionResponse> {
		...
		Logger.debug("[DietCodeAgent] newSession called:", {
			sessionId,
			cwd: params.cwd,
		...
		const session: DietCodeAcpSession = {
			sessionId,
			cwd: params.cwd,
		...
		await this.sendAvailableCommands(sessionId, controller).catch((error) => {
			Logger.debug("[DietCodeAgent] Failed to send available commands:", error)
		})
	...
	private async getSessionModelState(mode: Mode): Promise<acp.SessionModelState> {
		...
		// Use provider-specific model ID key (e.g., dietcode uses actModeOpenRouterModelId)
	...
	async unstable_setSessionModel(params: acp.SetSessionModelRequest): Promise<acp.SetSessionModelResponse> {
		...
		Logger.debug("[DietCodeAgent] unstable_setSessionModel called:", {
		...
		// Update model ID using provider-specific keys (e.g., dietcode uses actModeOpenRouterModelId)
	 ...
	 * 2. Set up internal dietcode state subsription
	 * 3. Initialize or continue dietcode task
	 * 4. Translate DietCodeMessages to ACP SessionUpdates
	 * 5. Handle permission requests for tools/commands
	 * 6. Return when dietcode task completes, is cancelled, or needs user input
	 */
	async prompt(params: acp.PromptRequest): Promise<acp.PromptResponse> {
		...
		Logger.debug("[DietCodeAgent] prompt called:", {
			sessionId: params.sessionId,
		...
		try {
			...
			if (isLoadedSession && !hasActiveTask) {
				...
				Logger.debug("[DietCodeAgent] Resuming loaded session:", params.sessionId)
				...
				Logger.debug("[DietCodeAgent] Continuing existing task:", controller.task.taskId)

				// Find the last ask message and respond to it
				const messages = controller.task.messageStateHandler.getDietCodeMessages()
				...
				if (lastAskMessage) {
					...
					Logger.debug("[DietCodeAgent] No pending ask found, starting new task")
					await controller.initTask(textContent, imageContent, fileResources)
				}
			} else {
				// Start new task
				Logger.debug("[DietCodeAgent] Starting new task")
				await controller.initTask(textContent, imageContent, fileResources)
			}

			// Subscribe to dietcodeMessages changes after task is created
			if (controller.task) {
				const onDietCodeMessagesChanged = (change: DietCodeMessageChange) => {
					this.handleDietCodeMessagesChanged(
						params.sessionId,
						sessionState,
						change,
						resolvePrompt,
						...
						Logger.debug("[DietCodeAgent] Error handling dietcodeMessagesChanged:", error)
					})
				}

				controller.task.messageStateHandler.on("dietcodeMessagesChanged", onDietCodeMessagesChanged)
				cleanupFunctions.push(() => {
					controller.task?.messageStateHandler.off("dietcodeMessagesChanged", onDietCodeMessagesChanged)
				})
			}
			...
			for (const cleanup of cleanupFunctions) {
				try {
					...
					Logger.debug("[DietCodeAgent] Error during cleanup:", error)
				}
			}
	...
	private async handleDietCodeMessagesChanged(
		sessionId: string,
		sessionState: AcpSessionState,
		change: DietCodeMessageChange,
		resolvePrompt: (response: acp.PromptResponse) => void,
		promptResolved: { value: boolean },
	): Promise<void> {
		Logger.debug("[DietCodeAgent] handleDietCodeMessagesChanged:", change)
		try {
			switch (change.type) {
				case "add":
	 ...
	 * 3. Responds to DietCode's ask based on the decision
	 *
	 * @param sessionId - The session ID
	 * @param sessionState - The session state
	 * @param message - The DietCode ask message
	 * @param permissionRequest - The permission request details from translateMessage
	 */
	private async handlePermissionRequest(
		sessionId: string,
		sessionState: AcpSessionState,
		message: DietCodeMessageType,
		permissionRequest: Omit<acp.RequestPermissionRequest, "sessionId">,
	): Promise<void> {
		const session = this.sessions.get(sessionId)

		if (!session) {
			Logger.debug("[DietCodeAgent] No session found for permission request")
			return
		}
		...
		if (!controller?.task) {
			Logger.debug("[DietCodeAgent] No active task for permission request")
			return
		}

		const askType = message.ask as DietCodeAsk

		try {
			// Request permission from the client
			const response = await this.requestPermission(sessionId, permissionRequest.toolCall, permissionRequest.options)

			Logger.debug("[DietCodeAgent] Permission response received:", response.outcome)
	 ...
	 *
	 * This method uses translateMessage to properly map DietCodeMessages to ACP SessionUpdates,
	...
	private async processMessageWithDelta(
		...
		if (isTextStreamingMessage && message.text) {
			...
			if (result.requiresPermission && result.permissionRequest && !message.partial) {
				...
				// This will request permission from the client and respond to DietCode
	...
	async cancel(params: acp.CancelNotification): Promise<void> {
		const session = this.sessions.get(params.sessionId)
		if (!session) {
			Logger.debug("[DietCodeAgent] cancel called for non-existent session:", params.sessionId)
			return
		}
		const sessionState = this.sessionStates.get(params.sessionId)

		Logger.debug("[DietCodeAgent] cancel called:", {
			sessionId: params.sessionId,
		...
		if (sessionState) {
			...
			if (controller?.task) {
				try {
					...
					Logger.debug("[DietCodeAgent] Error cancelling task:", error)
				}
			}
		}
	}
	...
	async setSessionMode(params: acp.SetSessionModeRequest): Promise<acp.SetSessionModeResponse> {
		...
		Logger.debug("[DietCodeAgent] setSessionMode called:", {
			sessionId: params.sessionId,
	 ...
	 * - DietCode account authentication (dietcode-oauth)
	...
	async authenticate(params: acp.AuthenticateRequest): Promise<acp.AuthenticateResponse> {
		Logger.debug("[DietCodeAgent] authenticate called:", { methodId: params.methodId })
		...
		if (params.methodId !== "dietcode-oauth") {
		...
		Logger.debug("[DietCodeAgent] AuthHandler enabled, getting callback URL...")

		// Get the callback URL first to ensure the server is ready
		let callbackUrl: string
		try {
			callbackUrl = await authHandler.getCallbackUrl("/auth")
			Logger.debug("[DietCodeAgent] Callback URL ready:", callbackUrl)
		} catch (error) {
			Logger.error("[DietCodeAgent] Failed to get callback URL:", error)
		...
		try {
			...
			Logger.debug("[DietCodeAgent] Starting OAuth flow...")

			// Start the OAuth flow - this opens the browser
			await authService.createAuthRequest()

			Logger.debug("[DietCodeAgent] Browser opened, waiting for callback...")
			...
			while (Date.now() - startTime < AUTH_TIMEOUT_MS) {
				...
				const authData = stateManager.getSecretKey("dietcode:dietcodeAccountId")
				if (authData) {
					Logger.debug("[DietCodeAgent] Authentication successful")

					// Set up the provider configuration for dietcode
					stateManager.setGlobalState("actModeApiProvider", "dietcode")
					stateManager.setGlobalState("planModeApiProvider", "dietcode")
					await stateManager.flushPendingState()
			...
			Logger.error("[DietCodeAgent] Authentication error:", error)
			throw error
		}
	}
	...
	private async emitSessionUpdate(sessionId: string, update: acp.SessionUpdate): Promise<void> {
		...
		try {
			...
			Logger.debug("[DietCodeAgent] Error emitting session update:", error)
	...
	protected async requestPermission(
		...
		Logger.debug("[DietCodeAgent] Requesting permission:", {
		...
		if (!this.permissionHandler) {
			...
			Logger.debug("[DietCodeAgent] No permission handler set, auto-rejecting")
	 ...
	 *
	 * This fetches commands from DietCode's slash command system, filters out
	...
	private async sendAvailableCommands(sessionId: string, controller: Controller): Promise<void> {
		try {
			// Get all available commands from DietCode
			...
			Logger.debug("[DietCodeAgent] Sent available commands:", {
				sessionId,
				commandCount: availableCommands.length,
				commands: availableCommands.map((c) => c.name),
			})
		} catch (error) {
			Logger.debug("[DietCodeAgent] Error sending available commands:", error)
		}
	}
	...
	private async authenticateOpenAiCodex(): Promise<acp.AuthenticateResponse> {
		Logger.debug("[DietCodeAgent] Starting OpenAI Codex OAuth flow...")

		try {
			// Get the authorization URL and start the callback server
			const authUrl = openAiCodexOAuthManager.startAuthorizationFlow()

			Logger.debug("[DietCodeAgent] Opening browser for OpenAI Codex auth:", authUrl)
			...
			Logger.debug("[DietCodeAgent] OpenAI Codex authentication successful")
			...
			Logger.error("[DietCodeAgent] OpenAI Codex authentication error:", error)
			throw error
		}
	}
}

...
import type { DietCodeSessionEvents } from "./public-types.js"

/**
 * Type-safe EventEmitter for DietCodeAgent session events.
 *
 ...
 * @example
 * ```typescript
 * const agent = new DietCodeAgent({ version: "1.0.0" })
 ...
 * ```
 */
export class DietCodeSessionEmitter {
	 ...
	 */
	on<K extends keyof DietCodeSessionEvents>(event: K, listener: DietCodeSessionEvents[K]): this {
	 ...
	 * @returns This emitter instance for chaining
	 */
	once<K extends keyof DietCodeSessionEvents>(event: K, listener: DietCodeSessionEvents[K]): this {
	 ...
	 * @returns This emitter instance for chaining
	 */
	off<K extends keyof DietCodeSessionEvents>(event: K, listener: DietCodeSessionEvents[K]): this {
	 ...
	 */
	emit<K extends keyof DietCodeSessionEvents>(event: K, ...args: Parameters<DietCodeSessionEvents[K]>): boolean {
	 ...
	 * @returns This emitter instance for chaining
	 */
	removeAllListeners<K extends keyof DietCodeSessionEvents>(event?: K): this {
		if (event) {
	 ...
	 */
	listenerCount<K extends keyof DietCodeSessionEvents>(event: K): number {

...
async function main() {
	log("\n\n\nStarting dietcode-core service...\n\n\n")
...
function setupHostProvider(extensionContext: any, extensionDir: string, dataDir: string) {
	...
	// dietcode-core expects the binaries to be unpacked in the directory where it is running.
...
/**
 * Gracefully shutdown the dietcode-core process by:
...
function showHelp() {
	Logger.log(`
DietCode Core - Standalone Server

Usage: node dietcode-core.js [options]
...
Options:
  ...
  -c, --config <path>            Directory for DietCode data storage (default: ~/.dietcode)

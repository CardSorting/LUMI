...
5. **ENGINEERING vs FINALIZATION**: \`attempt_completion\` verifies engineering work only. Documentation and Knowledge Ledger updates (.wiki/) run in the same session via \`run_finalization\` after engineering is verified — not via direct wiki writes or another \`attempt_completion\`.

...
export class DietCodeToolSet {
	...
	public static getNativeTools(variant: PromptVariant, context: SystemPromptContext) {
		...
		const mcpTools = mcpServers?.flatMap((server) => mcpToolToDietCodeToolSpec(variant.family, server))
...
/**
 * Convert an MCP server's tools to DietCodeToolSpec format
 */
export function mcpToolToDietCodeToolSpec(family: ModelFamily, server: McpServer): DietCodeToolSpec[] {

...
interface DietCodeHandlerOptions extends CommonApiHandlerOptions {
	...
	dietcodeAccountId?: string
	dietcodeApiKey?: string
}

const DIETCODE_FREE_MODELS = ["minimax/minimax-m2.5", "kwaipilot/kat-coder-pro", "z-ai/glm-5"]

export class DietCodeHandler implements ApiHandler {
	private options: DietCodeHandlerOptions
	private dietcodeAccountService = DietCodeAccountService.getInstance()
	...
	constructor(options: DietCodeHandlerOptions) {
		this.options = options
		this._authService = AuthService.getInstance()
	}

	private async ensureClient(): Promise<OpenAI> {
		const dietcodeAccountAuthToken = this.options.dietcodeApiKey || (await this._authService.getAuthToken())
		...
		if (!this.client) {
			try {
				...
				this.client = new OpenAI({
					...
					apiKey: dietcodeAccountAuthToken,
					defaultHeaders,
				...
				throw new Error(`Error creating DietCode client: ${error.message}`)
			}
		}
		// Ensure the client is always using the latest auth token
		this.client.apiKey = dietcodeAccountAuthToken
		return this.client
	}
	...
	async *createMessage(systemPrompt: string, messages: DietCodeStorageMessage[], tools?: OpenAITool[]): ApiStream {
		try {
			...
			for await (const chunk of stream) {
				Logger.debug(`DietCodeHandler chunk:${JSON.stringify(chunk)}`)
				// openrouter returns an error object instead of the openai sdk throwing an error
				if ("error" in chunk) {
					const error = chunk.error as OpenRouterErrorResponse["error"]
					Logger.error(`DietCode API Error: ${error?.code} - ${error?.message}`)
					// Include metadata in the error message if available
					const metadataStr = error.metadata ? `\nMetadata: ${JSON.stringify(error.metadata, null, 2)}` : ""
					throw new Error(`DietCode API Error ${error.code}: ${error.message}${metadataStr}`)
				}
				...
				if ((choice?.finish_reason as string) === "error") {
					...
					if (choiceWithError.error) {
						...
						Logger.error(`DietCode Mid-Stream Error: ${error.code || error.type || "Unknown"} - ${error.message}`)
						throw new Error(`DietCode Mid-Stream Error: ${error.code || error.type || "Unknown"} - ${error.message}`)
					}
					throw new Error(
						"DietCode Mid-Stream Error: Stream terminated with error status but no error details provided",
					)
				}
				...
				*/
				...
				if (!didOutputUsage && chunk.usage) {
					...
					const isFreeModel = DIETCODE_FREE_MODELS.includes(modelId)
			...
			if (!didOutputUsage) {
				Logger.warn("DietCode API did not return usage chunk, fetching from generation endpoint")
			...
			Logger.error("DietCode API Error:", error)
			throw error
		}
	}
	...
	async embedText(text: string): Promise<number[] | null> {
		try {
			...
			const response = await client.embeddings.create({
				model: "text-embedding-3-small", // Default embedding model for DietCode
				input: text,
			})
			return response.data[0].embedding
		} catch (error) {
			Logger.error("DietCode embedText error:", error)
			return null
		}
	}
	...
	async embedBatch(texts: string[]): Promise<(number[] | null)[]> {
		try {
			...
			Logger.error("DietCode embedBatch error:", error)
	...
	async getApiStreamUsage(): Promise<ApiStreamUsageChunk | undefined> {
		if (this.lastGenerationId) {
			try {
				...
				Object.assign(headers, await buildDietCodeExtraHeaders())

				const response = await axios.get(
					`${this.dietcodeAccountService.baseUrl}/generation?id=${this.lastGenerationId}`,
					{
						headers,
				...
				// ignore if fails
				Logger.error("Error fetching dietcode generation details:", error)
			}
		}
		return undefined
	}

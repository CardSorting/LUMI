...
export const SPIDER_AGENT_TOOL_SCHEMA = {
  ...
  inputSchema: {
    ...
    properties: {
      operation: {
        type: 'string',
        enum: ['check', 'preflight', 'preflightBundle', 'batchPreflight', 'audit', 'gate', 'gateBundle', 'resync', 'bundle', 'explain'],
        description: 'check=unified phase router; gateBundle=CI+agent payload',
      },
      phase: {
        type: 'string',

...
# This installs the dietcode-core app to the user's home directory,
...
CORE_DIR=~/.dietcode/core
INSTALL_DIR=$CORE_DIR/dev-instance/
LOG_FILE=~/.dietcode/dietcode-core-service.log
...
pkill -f dietcode-core.js || true

# Detect platform name using the same logic as DietCodeDirs.kt in the plugin.
...
echo pwd: $(pwd)
set -x
NODE_PATH=$BINARY_MODULES_DIR:./node_modules DEV_WORKSPACE_FOLDER=/tmp/ node dietcode-core.js 2>&1 | tee $LOG_FILE

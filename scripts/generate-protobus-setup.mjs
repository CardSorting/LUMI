...
async function generateWebviewProtobusClients(protobusServices) {
	...
	static override serviceName: string = "dietcode.${serviceName}"
${rpcs.join("\n")}
}`)
	}
...
async function generateVscodeServiceTypes(protobusServices) {
	...
	for (const [serviceName, def] of Object.entries(protobusServices)) {
		...
		for (const [rpcName, rpc] of Object.entries(def.service)) {
			...
			if (!rpc.responseStream) {
				servers.push(`     ${rpcName}:(controller: IController, request: ${requestType}) => Promise<${responseType}>`)
			} else {
				servers.push(
					`     ${rpcName}:(controller: IController, request: ${requestType}, responseStream: StreamingResponseHandler<${responseType}>, requestId?: string) => Promise<void>`,
				)
			}
		}
		servers.push(`}\n`)
...
async function generateVscodeProtobusServers(protobusServices) {
	...
	for (const [serviceName, def] of Object.entries(protobusServices)) {
		...
		servers.push(`} \n`)
		serviceMap.push(`    "dietcode.${serviceName}": ${serviceName}Handlers,`)
		imports.push("")
	}
...
async function generateStandaloneProtobusServiceSetup(protobusServices) {
	...
	for (const [name, def] of Object.entries(protobusServices)) {
		...
		handlerSetup.push(`    server.addService(dietcode.${name}Service, {`)
		for (const [rpcName, rpc] of Object.entries(def.service)) {
			imports.push(`import { ${rpcName} } from "@core/controller/${dir}/${rpcName}"`)
			const requestType = `dietcode.${rpc.requestType.type.name}`
			const responseType = `dietcode.${rpc.responseType.type.name}`
			if (rpc.requestStream) {
				throw new Error("Request streaming is not supported")
			}
			if (rpc.responseStream) {
...
import { dietcode } from "@generated/grpc-js"
import type { IController } from "@core/controller/types"
